...
const ToastItem = (toast: ToastItemProps) => {
    const { message, type, buttonLabel, onButtonClick, onClose, size = 'medium', animation = 'fade-in-up' } = toast
    const Icon = TOAST_ICON[type]
    // const typeClass = styles[type] ?? ''

    return (
        <div className={`${styles.toast} ${styles[type]} ${styles[size]} ${styles[animation]}`} role="alert">
